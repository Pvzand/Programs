...
c     calculates: 
c
c     < j, m, k | V(rvec) | jp, mp, kp >
c
c     note: only m=0 is implemented

c     input: j,m,k,jp,mp,kp : integers
c            rvec(3) real*8 position of He

c     based on Nadine's notes
...
c     Morse function
      ...
      double precision  m2min,m2max
      double precision  threeJ,thrcoef(ndim)

      integer myint,im2min,im2max
...
c_____check triangula condition      
      if (m1+m2+m3.ne.0)    return
      if (l3.lt.abs(l2-l1)) return 
      if (l3.gt.l2+l1)      return 

      call DRC3JM(al1,al2,al3,am1,m2min,m2max,thrcoef,ndim,ier)
      im2min= myint(m2min)
      im2max= myint(m2max)
c     write(*,*) 'min max',im2min,im2max
      threeJ=thrcoef(m2-im2min+1)
