...
SUBROUTINE WPINI_SOd(Y0,ndimy,iopt)
...
!!!  set the real part of the coefficient corresponding to surface ihop to 1:
     y0(ncoordmom+2*(ihop-1)+1) = 1.d0

  case(2) !!! the real parts of all the coefficients are set to 1/sqrt(nDIMorb)
     !!! initial surface for classical dynamics of nuclear coordinates chosen at random
     coef=1.d0/dsqrt(dfloat(nDIMorb))
     y0(ncoordmom+1:ncoordmomcoef:2) = coef
     IOUT = 6
!     iseed = 0 !!! to continue sampling from previous call to vranf
!     CALL VRANF(RANDHOP,nrand,iseed,IOUT)
  ...
  case(3) !!! impose one initial state (from input file, ihop, same line as iopt)
     ...
     y0(ncoordmom+2*(ihop-1)+1) = 1.d0   !!! set to 1 the real part of the coefficient corresponding to surface ihop
  ...
  print *, 'in wpini_so: initial coefficients: '
  print *,y0(ncoordmom+1:ncoordmomcoef)
  print *, 'in wpini_so: initial energy phases (should be zero): '
  print *,y0(ncoordmomcoef+1:nvar)
  
  RETURN
END SUBROUTINE WPINI_SOD
