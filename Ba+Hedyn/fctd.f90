...
!!! for common/masses/
  ...
  double complex, dimension(ndimorb) :: zdcoef, zddcoefdt   !!! complex d coefficients and t-derivatives
  double complex, dimension(ndimorb,ndimorb) :: zrdkj   !!! complex(rdkj_SOr,rdkj_SOi)
...
!  print *,' coefs d (Re,Im, for each d) ',y(ncoordmom+1:ncoordmomcoef)
!  print *,' coefs energy phases ',y(ncoordmomcoef+1:nvar)
!  call flush(6)
...
!!! (eigen Ecomm is also reordered accordingly in COMMON DIM)
!!!  if (ifail.ne.0) then ???
...
!!!  EQUATIONS OF MOTION for wave packet coefficients
...
!!! Time-derivatives for the d_k coefficients (real and imaginary parts)
  zdcoef = dcmplx(y(ncoordmom+1:ncoordmomcoef:2),y(ncoordmom+2:ncoordmomcoef:2))
  do i=1,ndimorb
     zddcoefdt(i)=-sum(zrdkj(i,:)*zdcoef*cdexp(dcmplx(0.d0,Ephase(i)-Ephase(:))))
  enddo
  dery(ncoordmom+1:ncoordmomcoef:2) = dreal(zddcoefdt)
  dery(ncoordmom+2:ncoordmomcoef:2) = dimag(zddcoefdt)
  
!!! Time-derivatives for the energy phases
  dery(ncoordmomcoef+1:nvar) = eigenE
