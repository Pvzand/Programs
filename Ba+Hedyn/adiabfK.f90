subroutine adiabfK(eigenSO,ndimeigenSO,splim, ifail, linit)
!!!
!!! This version SHOULD NOT be used in case of degeneracy between pairs !!!
!!! >>>>> NOT DEBUGGED
...
!!!  (eigvcoldRe,eigvcoldIm) with overlap >= splim
...
!!! >>>>> and eigenE is updated only at the end
  ...
  integer               :: il, ic, iv, ivp, ivd, ivp1, ivd1
  real (kind=8)         :: splim2
...
!!! real, imaginary part of eigenvectors; energies; for common DIM
...
!!! temporary storage for energies
  real (kind=8), dimension (ndimorb) :: tempE
...
!!! STEP 1 : first assign the obvious: levels with no crossing since the beginning
  ...
  enddo
  do iv = 1, ndimorb-1, 2
     if (scprodsq(iv,iv)+scprodsq(iv+1,iv+1).gt.splim2) then
        indx(iv) = iv
        indx(iv+1) = iv+1
        indxr(iv) = iv
        indxr(iv+1) = iv+1
        ...
        eigvcoldRe(:,iv) = eigvcRe(:,iv)
        eigvcoldIm(:,iv) = eigvcIm(:,iv)
        eigvcoldRe(:,iv+1) = eigvcRe(:,iv+1)
        eigvcoldIm(:,iv+1) = eigvcIm(:,iv+1)
     endif
  enddo
     
  if (nassign.ne.ndimorb) then   !!!
     ...
     do ivd = 1, ndimorb-1, 2
        if (indx(ivd).eq.0) then  ! level not assigned in step 1
           ...
           if (scprodsq(ivp,ivd)+scprodsq(ivp1,ivd)+scprodsq(ivp,ivd1)+scprodsq(ivp1,ivd1).gt.splim2) then  ! ivd matches iv from previous call
              ...
              eigenSO(ivp1) = tempE(ivd1)
                  
              eigvcRe(1:ndimorb,ivp) = dreal(zeigvctemp(1:ndimorb,ivd))
              eigvcIm(1:ndimorb,ivp) = dimag(zeigvctemp(1:ndimorb,ivd))
              eigvcRe(1:ndimorb,ivp1) = dreal(zeigvctemp(1:ndimorb,ivd1))
              eigvcIm(1:ndimorb,ivp1) = dimag(zeigvctemp(1:ndimorb,ivd1))
           endif
        endif
     enddo
  endif
     
  if (nassign.ne.ndimorb)  then     !  need for further re-ordering
!!! STEP 3
...
!!! calculate all the scalar products of the unassigned eigenvectors
     do ivd = 1, ndimorb-1, 2
        ...
        do ivp = 1, ndimorb-1, 2
           ...
           if (scprodsq(ivp,ivd)+scprodsq(ivp1,ivd)+scprodsq(ivp,ivd1)+scprodsq(ivp1,ivd1).gt.splim2) then  ! ivd matches ivp from previous call
              nassign = nassign + 2
              indx(ivd) = ivp
              indxr(ivp) = ivd
              indx(ivd+1) = ivp1
...
!!! save for next call, save for commons
     ...
     ifail = 1
     print *
     print *, ' >>>>> ERROR in adiabfK <<<<< '
     print *,'       Only nassign = ',nassign,' eigenvectors could be assigned out of ', ndimorb
     print *,' splim = ',splim
     print *,'   check that there is no degeneracy! '
  endif


  return
end subroutine adiabfK
