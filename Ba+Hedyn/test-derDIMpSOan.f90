...
do ipt = 1, npts
   ...
   enddo
   call DIMpdynan(xyzdist,rdist,HDIMp,ndimp,nHe,iunit)

   if (lderiv) then
      call DIMpderan(xyzdist,rdist,HDIMpder,ndimp,nHe,nxyz,iunit)
      ...
      do iHe = 1, nHe
         do ixyz = 1, 3
            ...
            call DIMpdynan(xyzdistp,rdistp,HDIMpp,ndimp,nHe,ireadpotexc)
            xyzdistm = xyzdist
            rdistm = rdist
            ...
            call DIMpdynan(xyzdistm,rdistm,HDIMpm,ndimp,nHe,ireadpotexc)
            do il = 1, 3
...
! copy HDIMp into the whole (double-sized) H matrix
   ...
   call SOmatpxyz(zHSO,ndimpSO,DeltaSO)
   zHSORe = dreal(zHSO)
   zHSOIm = dimag(zHSO)
...
! set Moption to 0 for only eigenvalues, to any other value for eigenvectors and eigenvalues
   Moption = 1
   ierr = 0
   call CH(ndimpSO,ndimpSO,zHDIMpSORe,zHDIMpSOIm,eigenSO,Moption,zeigvcRe,zeigvcIm,work1,work2,work3,ierr)
